...
export default function Dashboard() {
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-black text-white' : 'bg-gray-50 text-gray-900'} relative`}>
            <div className={`fixed top-[-20%] left-[-10%] w-[50%] h-[50%] blur-[150px] rounded-full pointer-events-none ${isDark ? 'bg-blue-900/30' : 'bg-blue-200/50'}`} />
            <div className={`fixed bottom-[-20%] right-[-10%] w-[50%] h-[50%] blur-[150px] rounded-full pointer-events-none ${isDark ? 'bg-indigo-900/20' : 'bg-indigo-200/40'}`} />

            <main className="z-10 flex flex-col w-full max-w-7xl mx-auto p-8 lg:min-h-[90vh]">
                ...
                <div className="flex flex-col lg:flex-row gap-8 mt-8 flex-1 min-h-[600px]">
                    {/* Left panel */}
                    <div className="w-full lg:w-1/3 flex flex-col gap-6 h-full">
                        <div className={`backdrop-blur-md p-6 rounded-2xl flex flex-col h-full shadow-xl transition-colors border ${isDark ? 'bg-gray-900/50 border-gray-800' : 'bg-white/90 border-gray-200 shadow-lg'}`}>
                            ...
                            <label className={`border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center transition-all cursor-pointer group mb-6 overflow-hidden relative min-h-[140px] flex-shrink-0 ${isDark ? 'border-gray-700 text-gray-400 hover:border-blue-500 hover:bg-gray-800/40' : 'border-gray-300 text-gray-500 hover:border-blue-500 hover:bg-blue-50/50'}`}>
                                <input
                                    type="file"
                                    className="hidden"
                                    ref={fileInputRef}
                            ...
                            <button
                                ...
                                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-4 px-4 rounded-xl shadow-lg shadow-blue-500/25 transition-all transform hover:scale-[1.02] active:scale-95 flex justify-center items-center gap-2 flex-shrink-0"
                            >
                    ...
                    {/* Right panel */}
                    <div className={`w-full lg:w-2/3 backdrop-blur-md p-6 rounded-2xl flex flex-col shadow-xl overflow-hidden transition-colors border h-full ${isDark ? 'bg-gray-900/40 border-gray-800' : 'bg-white/90 border-gray-200 shadow-lg'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pr-3 custom-scrollbar pb-4">
                            {reasoning.length === 0 ? (
                                <div className={`h-full flex items-center justify-center border-2 border-dashed rounded-xl min-h-[300px] ${isDark ? 'border-gray-800 text-gray-600' : 'border-gray-300 text-gray-400 bg-gray-50/50'}`}>
            ...
            <section className={`z-10 relative w-full border-t transition-colors mt-8 py-20 px-8 ${isDark ? 'border-gray-800 bg-gray-950/80' : 'border-gray-300 bg-white/50'}`}>
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                        <div>
                            <h3 className={`text-xl font-bold mb-6 flex items-center gap-2 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>The Original Research</h3>
                            <p className={`leading-relaxed mb-6 text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                This application is a full-stack, cloud-hosted replication of the architecture detailed in the 2026 ICLR paper <a href="https://arxiv.org/html/2503.18968v2" target="_blank" rel="noreferrer" className="text-blue-500 hover:underline font-bold inline-flex items-center">MedAgent-Pro <ExternalLink size={16} className="ml-1" /></a>.
                            </p>
                            <p className={`leading-relaxed text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                The original research introduced a hierarchical multi-modal AI system designed to solve complex medical reasoning tasks by dynamically generating diagnostic pathways and bridging pure language models with specialized visual interpretation tools. I engineered this project to prove that complex, academic AI loops can be deployed into highly scalable, modern web architectures specifically optimized for real-world clinical usage.
                            </p>
                        </div>

                        <div className={`p-8 rounded-3xl border shadow-lg ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
                            <h3 className={`text-xl font-bold mb-8 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>My Architectural Additions</h3>
                            <ul className={`space-y-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                <li className="flex items-start gap-4">
                                    <div className={`p-3 rounded-xl mt-1 ${isDark ? 'bg-indigo-900/50' : 'bg-indigo-100'}`}>
                                ...
                                <li className="flex items-start gap-4">
                                    <div className={`p-3 rounded-xl mt-1 ${isDark ? 'bg-blue-900/50' : 'bg-blue-100'}`}>
                                ...
                                <li className="flex items-start gap-4">
                                    <div className={`p-3 rounded-xl mt-1 ${isDark ? 'bg-emerald-900/50' : 'bg-emerald-100'}`}>
